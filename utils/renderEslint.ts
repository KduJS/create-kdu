...
addEslintDependency('eslint-plugin-kdu')
...
const config: ESLintConfig = {
  root: true,
  extends: ['plugin:kdu/kdu3-essential']
}
...
function configureEslint({ language, styleGuide, needsPrettier /*needsCypress, needsCypressCT*/ }) {
  switch (`${styleGuide}-${language}`) {
    ...
    case 'default-typescript':
      addEslintDependency('@kdujs/eslint-config-typescript')
      config.extends.push('eslint:recommended')
      config.extends.push('@kdujs/eslint-config-typescript/recommended')
      break
  ...
  if (needsPrettier) {
    ...
    addEslintDependency('@kdujs/eslint-config-prettier')
    config.extends.push('@kdujs/eslint-config-prettier')
  }
    ...
    deepMerge(existingPkg, {
      scripts: {
        ...
        lint: needsTypeScript
          ? 'eslint . --ext .kdu,.js,.jsx,.cjs,.mjs,.ts,.tsx,.cts,.mts --fix --ignore-path .gitignore'
          : 'eslint . --ext .kdu,.js,.jsx,.cjs,.mjs --fix --ignore-path .gitignore'
      },
